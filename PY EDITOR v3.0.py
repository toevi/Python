...
class PythonEditor(_TkBase):
    ...
    def open_file(self):
        ...
        path = filedialog.askopenfilename(defaultextension=".py",
    ...
    def schedule_update(self, event=None):
        """Planuje aktualizację edytora."""
        # Uniknięcie wielokrotnego uruchamiania po modyfikacji
        if hasattr(self, '_after_id') and self._after_id:
            self.after_cancel(self._after_id)
            
        self._after_id = self.after(250, self.update_editor)
        self.set_unsaved_changes()

    def update_editor(self):
        """Aktualizuje wszystkie elementy edytora."""
        self.update_line_numbers()
        self.highlight_syntax_and_whitespace_and_check_errors()
        self.text_widget.edit_modified(False)

    def update_line_numbers(self, event=None):
        """Aktualizuje numery linii na pasku."""
        line_numbers_content = ""
        line_count = int(self.text_widget.index('end-1c').split('.')[0])
        for i in range(1, line_count + 1):
            line_numbers_content += str(i) + "\n"
        
        self.line_number_bar.config(state="normal")
        self.line_number_bar.delete("1.0", tk.END)
        self.line_number_bar.insert("1.0", line_numbers_content)
        self.line_number_bar.config(state="disabled")

        # Synchronizacja przewijania
        first_visible_line = self.text_widget.yview()[0]
        self.line_number_bar.yview_moveto(first_visible_line)

    def on_scroll(self, *args):
        """Obsługuje przewijanie paska przewijania."""
        self.text_widget.yview(*args)
        self.update_line_numbers()

    def on_yscroll(self, *args):
        """Obsługuje przewijanie pola tekstowego."""
    ...
    def run_code(self):
        ...
        if not self.file_path:
            messagebox.showwarning("Ostrzeżenie", "Zapisz plik przed uruchomieniem.")
            self.save_as_file()
            if not self.file_path:
                return

        # Sprawdzanie i zapisywanie zmian przed uruchomieniem
        if self.unsaved_changes:
            self.save_file()
            if self.unsaved_changes:
                return

        # Upewnij się, że poprzedni proces jest zatrzymany
        self.stop_code()

        self.terminal_text.config(state="normal")
        self.terminal_text.delete("1.0", tk.END)
        self.terminal_text.insert(tk.END, f"> Uruchamianie pliku: {os.path.basename(self.file_path)}\n", "info")
        self.terminal_text.config(state="disabled")
        
        # Użycie ścieżki do interpretera Pythona, która uruchamia program
        python_executable = sys.executable
        
        try:
            self.process = subprocess.Popen([python_executable, self.file_path],
            ...
            # Wątek do odczytu wyjścia w tle, aby nie blokować interfejsu
            import threading
    ...
    def read_output(self):
        """Odczytuje wyjście z podprocesu w czasie rzeczywistym."""
        if self.process:
            while True:
            ...
            while True:
                ...
                if not line:
                    break
                self.terminal_text.config(state="normal")
                self.terminal_text.insert(tk.END, line, "error")
                self.terminal_text.see(tk.END)
                self.terminal_text.config(state="disabled")

            self.process.wait()
            self.terminal_text.config(state="normal")
            self.terminal_text.insert(tk.END, f"\n> Zakończono z kodem wyjścia {self.process.returncode}\n", "info")
            self.terminal_text.config(state="disabled")
    ...
    def open_system_terminal(self):
        """Otwiera terminal systemowy w katalogu pliku."""
        try:
            if self.file_path:
            ...
            self.status_bar.config(text=f"Otwarto terminal w: {directory}")
